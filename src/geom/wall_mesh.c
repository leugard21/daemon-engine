...
static void add_wall_segment(Vtx *verts, int *at, float x0, float z0, float x1,
                             float z1, float y0, float y1, float r, float g,
                             float b) {
  Vtx a = {x0, y0, z0, r, g, b};
  Vtx b0 = {x1, y0, z1, r, g, b};
  Vtx c = {x1, y1, z1, r, g, b};
  Vtx d = {x0, y1, z0, r, g, b};
  ...
  for (int i = 0; i < map->line_count; i++) {
    ...
    if (l->back_sector < 0) {
      ...
      if (up_top - up_bot > 0.0001f) {
        add_wall_segment(verts, &at, p0.x, p0.y, p1.x, p1.y, up_bot, up_top,
                         0.2f, 0.6f, 0.8f);
      }
    }
  }
